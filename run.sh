...
spark-submit --conf "spark.driver.extraJavaOptions=${log4j_setting}" --conf "spark.executor.extraJavaOptions=${log4j_setting}" --files log4j.properties --driver-class-path jars/* --py-files dependencies.zip run_etl.py etl /BASEA/courses.json courses '{"Id":"integer", "Name":"string"}'

spark-submit --conf "spark.driver.extraJavaOptions=${log4j_setting}" --conf "spark.executor.extraJavaOptions=${log4j_setting}" --files log4j.properties --driver-class-path jars/* --py-files dependencies.zip run_etl.py etl /BASEA/students.json students '{"Id":"string", "RegisteredDate":"timestamp", "State":"string", "City":"string", "UniversityId":"string", "CourseId":"string", "SignupSource":"string"}' '{"date_column": "RegisteredDate", "date_format":"yyyy-MM-dd HH:mm:ss"}'
...
#spark-submit --conf "spark.driver.extraJavaOptions=${log4j_setting}" --conf "spark.executor.extraJavaOptions=${log4j_setting}" --files log4j.properties --py-files dependencies.zip run_etl.py "pivot" "datalake/events" "user_categories"
